...
def make_scores_transformers(model, query, answers_bodies, head = 3):
    scores = []
    for ind, elem in enumerate(answers_bodies):
        ...
        attensions = answ[2][head][0][0].detach().numpy()# + answ[2][9][0][0].detach().numpy()
...
def run_baseline(output_dir = '/notebook/touche2021/', input_dir = '/notebook/touche/', input_file = 'topics.xml', head = 0):
    ...
    for ind_q, elem in enumerate(list_of_tuples):
        ...
        scores = make_scores_transformers(model, query, answers_bodies, head)
    ...
    with open('./transformers' + str(head) +'.qrels', 'w') as fp:
