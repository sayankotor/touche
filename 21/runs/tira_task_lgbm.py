...
# this configures LightGBM as LambdaMART
    ...
    rf_lgbm.fit(result, qrels_df_train, topics_and_results_Valid = result, qrelsValid = qrels_df)
    print ("fitted")
    answs = transform(rf_lgbm, test_ds)
    
    # lmart_l.learner.get_booster().save_model('lgb_ranker.txt', num_iteration=lmart_l.learner.get_booster().best_iteration)
    ...
    write_qrels(output_dir, "lgbm_ranker", answs)
    
    answs_test = transform(rf_lgbm, val_ds)
    write_qrels(output_dir, "lgbm_v1_test", answs_test)
...
if __name__ == "__main__":
    ...
    start = timer()
    run_baseline(output_dir = '/notebook/touche2021/output/', input_dir = "/notebook/touche2021/runs/", input_file = 'topics.xml')
    end = timer()
