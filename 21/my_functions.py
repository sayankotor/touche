...
#from help_tira_run import PretrainedTransformerMismatchedEmbedder1
...
class extractorRoberta:
    def __init__(self, my_device = torch.device('cuda:0'), model_name = 'roberta.hdf5', model_path = current_directory_path + '/external_pretrained_models/'):
        ...
        try:
            ...
            self.model = SimpleTagger(text_field_embedder=text_field_embedder, 
                      ...
                      label_encoding='IOB1')
            print ("model path ",self.model_path + self.model_name)
            self.model.load_state_dict(torch.load(self.model_path + self.model_name, map_location=my_device))
